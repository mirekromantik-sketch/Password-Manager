#include <iostream>
#include <fstream>
#include <vector>
#include <string>
#include <ctime>
#include <random>
#include <algorithm>

#include <sodium.h>
#include "json.hpp"

using json = nlohmann::json;
using namespace std;

/*Password generator*/

string generatePassword(int groups, int lengthPerGroup) {
    const string lower = "abcdefghijklmnopqrstuvwxyz";
    const string upper = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    const string numbers = "0123456789";
    const string symbols = "!@#$%^&*()_=+{}[];':|<>/";

    string all = lower + upper + numbers + symbols;

    static thread_local mt19937 rng(random_device{}());
    uniform_int_distribution<size_t> dist(0, all.size() - 1);

    string pwd;
    
    for (int g = 0; g < groups; ++g) {
        string group;
        group.reserve(lengthPerGroup);
        
        // Ensure each group has variety (if length allows)
        if (lengthPerGroup >= 4) {
            group += lower[rng() % lower.size()];
            group += upper[rng() % upper.size()];
            group += numbers[rng() % numbers.size()];
            group += symbols[rng() % symbols.size()];
            
            for (int i = 4; i < lengthPerGroup; ++i)
                group += all[dist(rng)];
        } else {
            // For short groups, just use random characters
            for (int i = 0; i < lengthPerGroup; ++i)
                group += all[dist(rng)];
        }
        
        shuffle(group.begin(), group.end(), rng);
        pwd += group;
        
        // Add dash between groups (but not after the last one)
        if (g < groups - 1)
            pwd += "-";
    }
    
    return pwd;
}

/*Crypto*/

vector<unsigned char> deriveKey(
    const string& master,
    const unsigned char* salt
) {
    vector<unsigned char> key(
        crypto_aead_xchacha20poly1305_ietf_KEYBYTES
    );

    if(crypto_pwhash(
        key.data(), key.size(),
        master.c_str(), master.size(),
        salt,
        crypto_pwhash_OPSLIMIT_MODERATE,
        crypto_pwhash_MEMLIMIT_MODERATE,
        crypto_pwhash_ALG_DEFAULT
    ) != 0) {
        throw runtime_error("Key derivation failed");
    }

    return key;
}

vector<unsigned char> encrypData(
    const string& plaintext,
    const vector<unsigned char>& key,
    unsigned char* nonce
) {
    vector<unsigned char> ciphertext(
        plaintext.size() + crypto_aead_xchacha20poly1305_ietf_ABYTES
    );

    unsigned long long len;
    randombytes_buf(nonce, crypto_aead_xchacha20poly1305_ietf_NPUBBYTES);

    crypto_aead_xchacha20poly1305_ietf_encrypt(
        ciphertext.data(), &len,
        (const unsigned char*)plaintext.data(), plaintext.size(),
        nullptr, 0, nullptr,
        nonce, key.data()
    );

    ciphertext.resize(len);
    return ciphertext;
}

string decryptData(
    const vector<unsigned char>& ciphertext,
    const vector<unsigned char>& key,
    const unsigned char* nonce
) {
    vector<unsigned char> plaintext(
        ciphertext.size() - crypto_aead_xchacha20poly1305_ietf_ABYTES
    );

    unsigned long long len;

    if (crypto_aead_xchacha20poly1305_ietf_decrypt(
            plaintext.data(), &len,
            nullptr,
            ciphertext.data(), ciphertext.size(),
            nullptr, 0,
            nonce, key.data()
        ) != 0) {
            throw runtime_error("Wrong password or corrupted vault");
        }

    return string(plaintext.begin(), plaintext.begin() + len);
}

/*Vault I/O*/

void writeVault(
    const string& filename,
    const unsigned char* salt,
    const unsigned char* nonce,
    const vector<unsigned char>& data
) {
    ofstream f(filename, ios::binary | ios::trunc);
    f.write((char*)salt, crypto_pwhash_SALTBYTES);
    f.write((char*)nonce, crypto_aead_xchacha20poly1305_ietf_NPUBBYTES);
    f.write((char*)data.data(), data.size());
}

bool readVault(
    const string& filename,
    unsigned char* salt,
    unsigned char* nonce,
    vector<unsigned char>&data
) {
    ifstream f(filename, ios::binary);
    if(!f) return false;

    f.read((char*)salt, crypto_pwhash_SALTBYTES);
    f.read((char*)nonce, crypto_aead_xchacha20poly1305_ietf_NPUBBYTES);

    data.assign(
        istreambuf_iterator<char>(f),
        istreambuf_iterator<char>()
    );

    return true;
}

/*Main*/

int main() {
    if(sodium_init() < 0) {
        cerr << "libsodium init failed";
        return 1;
    }
    while(true) {
        cout << "1. Create / add passwords\n";
        cout << "2. Open Vault\n";
        cout << "3. Quit\n";
        
        int choice;
        cin >> choice;
        cin.ignore();

        if(choice == 1) {
            int groups, len;
            cout << "Groups: "; cin >> groups;
            cout << "Length per group: "; cin >> len;  // Fixed typo, added colon
            cin.ignore();

            json vault = json::array();

            unsigned char salt[crypto_pwhash_SALTBYTES];
            unsigned char nonce[crypto_aead_xchacha20poly1305_ietf_NPUBBYTES];
            vector<unsigned char> ciphertext;

            string master;
            cout << "Master password: ";
            getline(cin, master);

            bool vaultExists = readVault("Passwords.vault", salt, nonce, ciphertext);

            if(vaultExists) {
                try {
                    auto key = deriveKey(master, salt);
                    string plaintext = decryptData(ciphertext, key, nonce);
                    vault = json::parse(plaintext);
                    cout << "Existing vault loaded. Adding new passwords... \n";
                } catch(const exception& e) {
                    cerr << "Error: " << e.what() << "\n";
                    cerr << "Starting fresh vault instead.\n";
                    vault = json::array();
                    randombytes_buf(salt, sizeof salt);
                }
            } else{
                cout << "Creating new vault...\n";
                randombytes_buf(salt, sizeof salt);
            }
            
            // Generate ONE password with the group format
            time_t now = time(nullptr);
            vault.push_back({
                {"password", generatePassword(groups, len)},  // Pass both parameters
                {"timestamp", ctime(&now)}
            });

            auto key = deriveKey(master, salt);
            auto encrypted = encrypData(vault.dump(4), key, nonce);

            writeVault("Passwords.vault", salt, nonce, encrypted);
            cout << "Vault encrypted and saved. \n";
            continue;
        }

        else if (choice == 2) {
            unsigned char salt[crypto_pwhash_SALTBYTES];
            unsigned char nonce[crypto_aead_xchacha20poly1305_ietf_NPUBBYTES];
            vector<unsigned char> ciphertext;

            if(!readVault("Passwords.vault", salt, nonce, ciphertext)) {
                cerr << "Vault not found\n";
                continue;  
            }

            string master;  
            cout << "Master Password: ";  
            getline(cin, master);  

            try {  
                auto key = deriveKey(master, salt);
                string plaintext = decryptData(ciphertext, key, nonce);
                
                json vault = json::parse(plaintext);
                cout << "\n=== Vault Contents ===\n";
                for (auto& e : vault) {
                    string ts = e["timestamp"];
                    
                    if (!ts.empty() && ts.back() == '\n') {
                        ts.pop_back();
                    }
                    cout << "Password: " << e["password"].get<string>()  << " | " << ts << "\n";
                }
            } catch(const exception& e) {
                cerr << "Error: " << e.what() << "\n";
            }
            continue;
        }

        if(choice == 3){
            return 1;
        }
    }
    getchar();
    return 0;
}